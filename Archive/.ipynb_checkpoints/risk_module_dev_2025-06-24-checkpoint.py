...
# ─── Test Block: Factor Calculations ─────────────────────────────────────
...
# ─── Test Block: Factor Calculations Verification ─────────────────────────────────────
...
# ─── Test Block: Single Stock Risk Calculations ─────────────────────────────────────
...
# ─── CELL: Beta Calculation Diagnostic for Exact Date Window ─────────────────────
...
def build_portfolio_view(
    ...
    """
    Run full portfolio-level risk diagnostics and factor decomposition.

    Includes:
      - Monthly return aggregation
      - Risk metrics (volatility, beta, idiosyncratic vol)
      - Factor loadings and volatilities
      - Weighted factor variance decomposition
      - Idiosyncratic risk attribution
      - Industry-level variance contribution
      - Full output summary for downstream reporting

    Args:
        weights (Dict[str, float]): Portfolio weights by ticker.
        start_date (str): Start of analysis window (YYYY-MM-DD).
        end_date (str): End of analysis window (YYYY-MM-DD).
        expected_returns (Optional[Dict[str, float]]): Manual expected return input per ticker.
        stock_factor_proxies (Optional[Dict]): Factor mapping per stock for:
            - market ETF
            - style ETFs (momentum/value)
            - industry ETF
            - subindustry peer list

    Returns:
        Dict[str, Any]: Structured output including:
            - allocations
            - volatility and covariance matrices
            - risk decomposition (idio, factor, total)
